...
def _build_prompt(db_path: str, query: str, options: argparse.Namespace) -> str:
    ...
    if result.returncode != 0:
        raise SystemExit(result.stderr or result.stdout or "agent-memory claude-prompt failed")
...
def main() -> None:
    ...
    parser.add_argument("--claude-bin", default=os.environ.get("AGENT_MEMORY_CLAUDE_BIN", "claude"))
    parser.add_argument("--max-turns", type=int, default=1)
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--extra-claude-arg", action="append", default=[])
    args = parser.parse_args()

    prompt = _build_prompt(args.db_path, args.query, args)
    full_prompt = f"{prompt}\n\nUser request:\n{args.query}" if prompt else args.query
    command = shlex.split(args.claude_bin)
    command.extend(["-p", full_prompt, "--max-turns", str(args.max_turns)])
    command.extend(args.extra_claude_arg)
