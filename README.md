...
The next consolidation layer is an experimental trace substrate, not automatic memory creation. `experience_traces` stores low-cost local event traces behind explicit write APIs only; there is no public trace CLI or Hermes hook trace write yet. Trace rows store fingerprints, sanitized summaries/signals, related memory/observation refs, and retention metadata. They intentionally do not add raw prompt/transcript columns and do not change approved-memory retrieval or ranking.
