...
The next consolidation layer is an experimental trace substrate, not automatic memory creation. `experience_traces` stores low-cost local event traces behind explicit write APIs and the manual `traces record` CLI only; Hermes hook trace writes are still disabled until a later opt-in slice. Trace rows store fingerprints, sanitized summaries/signals, related memory/observation refs, and retention metadata. They intentionally do not add raw prompt/transcript columns and do not change approved-memory retrieval or ranking. `traces list` is read-only JSON and supports `--surface`, `--event-kind`, and `--scope` filters for local dogfood.
