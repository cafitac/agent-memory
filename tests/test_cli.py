...
def test_dogfood_lifecycle_bounded_batch_operator_packet_bundles_readiness_inventory_and_commands(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "lifecycle-batch-operator-packet.db"
    output_path = tmp_path / "lifecycle-batch-operator-packet.json"
    initialize_database(db_path)
    source = ingest_source_text(
        db_path=db_path,
        source_type="note",
        content="operator packet source token=SHOULD_NOT_LEAK",
    )
    first = create_candidate_fact(
        db_path=db_path,
        subject_ref="operator packet fact one",
        predicate="needs",
        object_ref_or_value="reinforcement",
        evidence_ids=[source.id],
        scope="project:operator-packet",
        confidence=0.9,
    )
    second = create_candidate_fact(
        db_path=db_path,
        subject_ref="operator packet fact two",
        predicate="needs",
        object_ref_or_value="reinforcement",
        evidence_ids=[source.id],
        scope="project:operator-packet",
        ...
        for index in range(4):
            connection.execute(
                ...
                (
                    ...
                    "tester",
                    "a" * 64,
                    str(tmp_path / f"operator-prior-backup-{index}.db"),
                    "b" * 64,
                    json.dumps({"default_retrieval_mutated": False}),
                ),
            )
        for candidate_id, fact_id in [("operator-candidate-one", first.id), ("operator-candidate-two", second.id)]:
            connection.execute(
                """
                INSERT INTO g5_trace_candidate_reviews (
                    candidate_id, status, proposal_type, target_ref, cluster_json, cluster_sha256,
    ...
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_lifecycle_bounded_batch_operator_packet"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["default_retrieval_unchanged"] is True
    assert payload["quality_gate"] == {
    ...
    assert payload["candidate_inventory"] == {
        ...
        "already_applied_count": 0,
        "max_apply": 2,
        "selected_count_for_preview": 2,
        "candidate_json_included": False,
    }
    assert payload["operator_checklist"]["required_policy"] == "g5-lifecycle-reinforcement-apply-v1"
    assert payload["operator_checklist"]["required_batch_approval_phrase"] == "apply-approved-g5-lifecycle-bounded-batch-v1"
    assert "lifecycle-bounded-batch-apply" in payload["manual_apply_command_preview"]
    assert "lifecycle-bounded-batch-post-apply-verification" in payload["post_apply_verification_command_template"]
    assert payload["runbook_contract"]["manual_apply_command_contains_all_required_flags"] is True
    assert payload["runbook_contract"]["post_apply_verification_template_contains_all_required_flags"] is True
    assert payload["safety_exclusions"]["ordinary_conversation_auto_approval"] is False
    assert payload["safety_exclusions"]["broad_background_apply"] is False
    assert _table_counts(db_path, ["facts", "relations", "g5_trace_candidate_reviews", "g5_trace_candidate_applications"]) == before_counts
    assert "SHOULD_NOT_LEAK" not in result.stdout
...
def _write_ordinary_turn_inferred_evidence_rollup_report(
    path: Path,
    *,
    pass_gate: bool = True,
        ...
        json.dumps(
            {
                ...
                "evidence_summary": {
                    ...
                    "applied_memory_count": green_report_count,
                    "unique_trace_ref_count": green_report_count,
                    "unique_memory_ref_count": green_report_count,
                    "min_green_reports": 2,
                    "ready_for_broader_design": pass_gate,
                    "apply_supported": False,
                    "apply_executed": False,
                },
                "quality_gate": {
...
def _write_ordinary_turn_auto_approval_readiness_report(
    path: Path,
    *,
    pass_gate: bool = True,
    ...
    blocked = blocked_reasons or ([] if pass_gate else ["secret_like_ordinary_turns_present"])
    path.write_text(
        json.dumps(
            {
                ...
                "risk_counts": {
                    "ordinary_preference_like_turns": 2,
                    "secret_like_ordinary_turns": secret_like_count,
                    "ordinary_turns_without_explicit_remember_intent": 4,
                },
                "readiness_score": {
                    ...
                    "components": {
                        "explicit_ready_evidence": True,
                        "ordinary_turns_observed": True,
                        "secret_like_ordinary_turns_absent": secret_like_count == 0,
                        "apply_verifier_available": True,
                    },
                },
                "quality_gate": {
...
def test_dogfood_ordinary_turn_broader_automation_readiness_is_design_only(
    tmp_path: Path,
) -> None:
    rollup_path = tmp_path / "ordinary-turn-inferred-evidence-rollup.json"
    auto_path = tmp_path / "ordinary-turn-auto-approval-readiness.json"
    output_path = tmp_path / "ordinary-turn-broader-automation-readiness.json"
    _write_ordinary_turn_inferred_evidence_rollup_report(rollup_path)
    _write_ordinary_turn_auto_approval_readiness_report(auto_path)
    ...
    result = subprocess.run(
        [
            ...
            "--inferred-evidence-rollup",
            str(rollup_path),
            "--auto-approval-readiness",
            str(auto_path),
            "--min-rollup-green-reports",
            "2",
            "--min-readiness-score-percent",
            "100",
            "--output",
            str(output_path),
        ],
    ...
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_broader_automation_readiness"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["default_retrieval_unchanged"] is True
    assert payload["ordinary_conversation_auto_approval"] is False
    assert payload["broader_automation_policy"] == {
    ...
    assert payload["evidence_summary"] == {
        "inferred_rollup_green_report_count": 2,
        "inferred_rollup_applied_memory_count": 2,
        "ordinary_turn_readiness_score_percent": 100,
        "ordinary_turn_secret_like_count": 0,
        "ordinary_preference_like_turn_count": 2,
    }
    assert payload["forbidden_authority"] == {
        "executes_apply": False,
        "ordinary_conversation_auto_approval": False,
...
def test_dogfood_ordinary_turn_broader_automation_readiness_blocks_unsafe_inputs(
    tmp_path: Path,
) -> None:
    rollup_path = tmp_path / "ordinary-turn-inferred-evidence-rollup-red.json"
    auto_path = tmp_path / "ordinary-turn-auto-approval-readiness-red.json"
    output_path = tmp_path / "ordinary-turn-broader-automation-readiness-red.json"
    _write_ordinary_turn_inferred_evidence_rollup_report(
        rollup_path,
        pass_gate=False,
        green_report_count=1,
        blocked_reasons=["green_report_count_below_minimum"],
    )
    _write_ordinary_turn_auto_approval_readiness_report(
        auto_path,
        pass_gate=False,
        score_percent=50,
        secret_like_count=1,
    ...
    assert payload["quality_gate"] == {
        "pass": False,
        "decision": "ordinary_turn_broader_automation_not_ready_keep_blocked",
        "blocked_reasons": [
            "auto_approval_readiness_gate_not_green",
            "inferred_evidence_rollup_gate_not_green",
            "ordinary_turn_readiness_score_below_minimum",
            "rollup_green_report_count_below_minimum",
            "secret_like_ordinary_turns_present",
        ],
    }
    assert payload["broader_automation_policy"]["ready_for_design"] is False
    assert payload["broader_automation_policy"]["default_background_auto_approval_allowed"] is False
    assert payload["ordinary_conversation_auto_approval"] is False



def _write_ordinary_turn_broader_automation_readiness_report(
    path: Path,
    *,
    pass_gate: bool = True,
    ready_for_design: bool = True,
    readiness_score: int = 100,
    green_report_count: int = 2,
    secret_like_count: int = 0,
    blocked_reasons: list[str] | None = None,
) -> None:
    blocked = blocked_reasons or ([] if pass_gate else ["ordinary_turn_broader_automation_not_ready"])
    path.write_text(
        json.dumps(
            {
                ...
                "evidence_summary": {
                    ...
                    "inferred_rollup_applied_memory_count": green_report_count,
...
def test_dogfood_ordinary_turn_default_automation_policy_gate_is_exact_read_only(
    tmp_path: Path,
) -> None:
    readiness_path = tmp_path / "ordinary-turn-broader-automation-readiness.json"
    output_path = tmp_path / "ordinary-turn-default-automation-policy-gate.json"
    _write_ordinary_turn_broader_automation_readiness_report(readiness_path)
    ...
    result = subprocess.run(
        [
            ...
            "--broader-automation-readiness",
            str(readiness_path),
            "--policy",
            "ordinary-turn-default-automation-policy-v1",
            "--min-independent-green-windows",
            "2",
    ...
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_policy_gate"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["default_retrieval_unchanged"] is True
    assert payload["ordinary_conversation_auto_approval"] is False
    ...
    assert payload["policy_contract"] == {
        ...
        "min_independent_green_windows": 2,
    ...
    assert payload["evidence_requirements"] == {
        "broader_readiness_green": True,
        "readiness_score_percent": 100,
        "inferred_rollup_green_report_count": 2,
        "secret_like_ordinary_turn_count": 0,
        "min_independent_green_windows": 2,
    }
    assert payload["forbidden_authority"] == {
        "executes_apply": False,
...
def test_dogfood_ordinary_turn_default_automation_policy_gate_blocks_red_readiness(
    tmp_path: Path,
) -> None:
    readiness_path = tmp_path / "ordinary-turn-broader-automation-readiness-red.json"
    _write_ordinary_turn_broader_automation_readiness_report(
        readiness_path,
        pass_gate=False,
    ...
    assert payload["quality_gate"] == {
        "pass": False,
        "decision": "ordinary_turn_default_automation_policy_not_ready_keep_default_blocked",
        "blocked_reasons": [
            "broader_automation_readiness_gate_not_green",
            "broader_automation_readiness_not_ready_for_design",
            "inferred_rollup_green_report_count_below_policy_minimum",
            "ordinary_turn_readiness_score_below_policy_minimum",
            "secret_like_ordinary_turns_present",
        ],
    }
    assert payload["policy_contract"]["ready_for_opt_in_dry_run"] is False
    assert payload["policy_contract"]["default_auto_approval_enabled"] is False
    assert payload["ordinary_conversation_auto_approval"] is False



def _write_ordinary_turn_default_automation_policy_gate_report(
    path: Path,
    *,
    pass_gate: bool = True,
    ready_for_opt_in_dry_run: bool = True,
    blocked_reasons: list[str] | None = None,
) -> None:
    blocked = blocked_reasons or ([] if pass_gate else ["policy_gate_not_ready"])
    path.write_text(
        json.dumps(
            {
                ...
                "evidence_requirements": {
                    "broader_readiness_green": pass_gate,
                    "readiness_score_percent": 100 if pass_gate else 80,
                    "inferred_rollup_green_report_count": 2 if pass_gate else 1,
                    "secret_like_ordinary_turn_count": 0 if pass_gate else 1,
                    "min_independent_green_windows": 2,
                },
                "policy_contract": {
                    "policy": "ordinary-turn-default-automation-policy-v1",
...
def test_dogfood_ordinary_turn_default_automation_dry_run_lists_ref_safe_candidates(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "ordinary-turn-default-automation-dry-run.db"
    gate_path = tmp_path / "ordinary-turn-default-automation-policy-gate.json"
    output_path = tmp_path / "ordinary-turn-default-automation-dry-run.json"
    initialize_database(db_path)
    ...
    insert_experience_trace(
        ...
        event_kind="turn",
        content_sha256="b" * 64,
        summary="User password=SHOULD_NOT_LEAK.",
        scope="project:g2",
        retention_policy="ephemeral",
        metadata={"ordinary_turn": True},
    )
    ...
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_dry_run"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["ordinary_conversation_auto_approval"] is False
    assert payload["quality_gate"] == {
    ...
    assert payload["candidate_counts"] == {
        "ordinary_turn_count": 3,
        "eligible_preference_candidate_count": 1,
        "selected_candidate_count": 1,
        "blocked_secret_like_count": 1,
        "non_preference_memory_worthy_count": 0,
        "deferred_candidate_count": 0,
    }
    assert payload["candidate_refs"] == ["experience_trace:1"]
    assert payload["candidate_preview"][0]["memory_shape"] == "preference"
    assert payload["candidate_preview"][0]["summary_sha256"] == hashlib.sha256(
        b"User prefers concise terminal summaries."
    ).hexdigest()
    assert payload["policy_contract"]["apply_supported"] is False
    assert payload["policy_contract"]["default_auto_approval_enabled"] is False
    assert payload["forbidden_authority"]["ordinary_conversation_auto_approval"] is False
    assert payload["privacy"] == {
        "raw_trace_summary_included": False,
...
def test_dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_preflight_is_status_only(
    tmp_path: Path,
) -> None:
    materialize_report = tmp_path / "disabled-recurring-scheduler-config-materialize.json"
    config_path = tmp_path / "ordinary-turn-default-automation-recurring-scheduler.disabled.json"
    output_path = tmp_path / "enabled-recurring-scheduler-config-preflight.json"
    config_payload = {
        ...
        json.dumps(
            {
                ...
                "default_retrieval_unchanged": True,
                "ordinary_conversation_auto_approval": False,
                "source_report": {"sha256": "c" * 64, "quality_gate_pass": True},
                "materialized_config": {
    ...
    result = subprocess.run(
        [
            ...
            "agent_memory.api.cli",
            "dogfood",
            "ordinary-turn-default-automation-enabled-recurring-scheduler-config-preflight",
            "--disabled-config-materialize-report",
            ...
            "ordinary-turn-default-automation-policy-v1",
            "--approval-phrase",
            "preflight-enabled-recurring-default-automation-scheduler-config-v1",
            "--output",
            str(output_path),
        ],
        cwd=Path(__file__).resolve().parents[1],
    ...
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_preflight"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["default_retrieval_unchanged"] is True
    assert payload["enabled_config_preflight"] == {
        "disabled_config_materialization_green": True,
        "disabled_config_file_present": True,
        "disabled_config_sha_matches_report": True,
        "disabled_config_currently_disabled": True,
        "fresh_evidence_requirements_present": True,
        "ci_health_watch_required": True,
        "rollback_evidence_required": True,
        "enablement_requires_separate_materialization": True,
        "background_or_cron_requires_separate_approval": True,
        "scheduler_cycle_not_executed": True,
        "apply_not_executed": True,
        "ready_for_enabled_config_contract_design_only": True,
    }
    assert payload["quality_gate"] == {
        "pass": True,
        "decision": "ordinary_turn_default_automation_enabled_recurring_scheduler_config_preflight_green_design_only",
        "blocked_reasons": [],
    }
    ...
    assert payload["approval_boundary"] == {
        "approval_phrase_consumed_for_preflight_only": True,
        "current_preflight_writes_enabled_config": False,
    ...
    tampered_config = dict(config_payload)
    tampered_config["enabled"] = True
    tampered_config["recurring_scheduler_enabled"] = True
    ...
    assert red_payload["quality_gate"]["decision"] == (
        "ordinary_turn_default_automation_enabled_recurring_scheduler_config_preflight_red_keep_recurring_blocked"
    )
    assert "scheduler_config_enabled_before_preflight" in red_payload["quality_gate"]["blocked_reasons"]
    assert "scheduler_config_recurring_scheduler_enabled_invalid" in red_payload["quality_gate"]["blocked_reasons"]
    assert red_payload["automation_authority"]["writes_scheduler_config"] is False
    assert red_payload["automation_authority"]["executes_scheduler_cycle"] is False


def test_dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_contract_is_data_only(
    tmp_path: Path,
) -> None:
    preflight_report = tmp_path / "enabled-recurring-scheduler-config-preflight.json"
    output_path = tmp_path / "enabled-recurring-scheduler-config-contract.json"
    preflight_report.write_text(
        json.dumps(
            {
                "kind": "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_preflight",
                "read_only": True,
                "mutated": False,
                "default_retrieval_unchanged": True,
                "ordinary_conversation_auto_approval": False,
                "source_report": {"sha256": "d" * 64, "quality_gate_pass": True},
                "scheduler_config": {
                    "path": str((tmp_path / "ordinary-turn-default-automation-recurring-scheduler.disabled.json").resolve(strict=False)),
                    "exists": True,
                    "sha256": "e" * 64,
                    "report_sha256": "e" * 64,
                    "enabled": False,
    ...
    result = subprocess.run(
        [
            ...
            "--enabled-config-preflight-report",
            str(preflight_report),
            "--policy",
            "ordinary-turn-default-automation-policy-v1",
            "--approval-phrase",
            "approve-enabled-recurring-default-automation-scheduler-config-contract-v1",
            "--output",
            str(output_path),
        ],
        cwd=Path(__file__).resolve().parents[1],
    ...
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_contract"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["config_contract"] == {
...
def test_dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_validate_is_fail_closed(
    tmp_path: Path,
) -> None:
    contract_report = tmp_path / "enabled-recurring-scheduler-config-contract.json"
    output_path = tmp_path / "enabled-recurring-scheduler-config-validate.json"
    contract_payload = {
    ...
    contract_report.write_text(json.dumps(contract_payload), encoding="utf-8")
    ...
    result = subprocess.run(
        [
            ...
            "agent_memory.api.cli",
            "dogfood",
            "ordinary-turn-default-automation-enabled-recurring-scheduler-config-validate",
            "--enabled-config-contract-report",
            str(contract_report),
            "--policy",
            "ordinary-turn-default-automation-policy-v1",
            "--output",
    ...
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_validate"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["validation"]["contract_green"] is True
    assert payload["validation"]["enabled_config_materialization_ready"] is True
    assert payload["validation"]["background_or_cron_still_blocked"] is True
    assert payload["automation_authority"] == {
        "executes_scheduler_cycle": False,
        "executes_apply": False,
        "recurring_scheduler_enabled": False,
    ...
    assert payload["quality_gate"] == {
        "pass": True,
        "decision": "ordinary_turn_default_automation_enabled_recurring_scheduler_config_validate_green_materialization_design_ready",
        "blocked_reasons": [],
    }
    assert payload["recommended_next_step"] == (
        "materialize_enabled_recurring_scheduler_config_after_separate_approval; do_not_start_background_cron"
    )

    tampered_report = tmp_path / "tampered-enabled-recurring-scheduler-config-contract.json"
    tampered_payload = json.loads(json.dumps(contract_payload))
    tampered_payload["config_contract"]["background_or_cron_enabled"] = True
    tampered_payload["config_contract"]["max_candidates_per_cycle"] = 5
    tampered_payload["automation_authority"]["writes_scheduler_config"] = True
    tampered_report.write_text(json.dumps(tampered_payload), encoding="utf-8")
    ...
    red_payload = json.loads(tampered_result.stdout)
    assert red_payload["read_only"] is True
    assert red_payload["mutated"] is False
    assert red_payload["validation"]["enabled_config_materialization_ready"] is False
    assert red_payload["quality_gate"]["pass"] is False
    assert red_payload["quality_gate"]["decision"] == (
        "ordinary_turn_default_automation_enabled_recurring_scheduler_config_validate_red_keep_materialization_blocked"
    )
    assert "contract_background_or_cron_enabled_invalid" in red_payload["quality_gate"]["blocked_reasons"]
    assert "contract_max_candidates_per_cycle_invalid" in red_payload["quality_gate"]["blocked_reasons"]
    assert "contract_authority_writes_scheduler_config_invalid" in red_payload["quality_gate"]["blocked_reasons"]




def test_dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_materialize_writes_enabled_config_only(
    tmp_path: Path,
) -> None:
    validate_report = tmp_path / "enabled-recurring-scheduler-config-validate.json"
    scheduler_config = tmp_path / "ordinary-turn-default-automation-recurring-scheduler.enabled.json"
    output_path = tmp_path / "enabled-recurring-scheduler-config-materialize.json"
    validate_report.write_text(
        json.dumps(
            {
                "kind": "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_validate",
                "read_only": True,
                "mutated": False,
                "default_retrieval_unchanged": True,
                "ordinary_conversation_auto_approval": False,
                ...
                "validation": {
                    ...
                    "expected_policy": "ordinary-turn-default-automation-policy-v1",
                    "target_state": "enabled",
                    "enabled_config_materialization_ready": True,
                    "background_or_cron_still_blocked": True,
                    "scheduler_cycle_not_executed": True,
                    "apply_not_executed": True,
                    "scheduler_config_not_written": True,
                },
                "approval_boundary": {
                    "current_validation_writes_enabled_config": False,
                    "current_validation_executes_scheduler_cycle": False,
                    "later_enabled_config_materialization_requires_separate_approval": True,
                    "later_background_or_cron_requires_separate_approval": True,
                },
                "automation_authority": {
    ...
    result = subprocess.run(
        [
            ...
            "agent_memory.api.cli",
            "dogfood",
            "ordinary-turn-default-automation-enabled-recurring-scheduler-config-materialize",
            "--enabled-config-validation-report",
            str(validate_report),
            "--scheduler-config",
            ...
            "ordinary-turn-default-automation-policy-v1",
            "--approval-phrase",
            "materialize-enabled-recurring-default-automation-scheduler-config-v1",
            "--output",
            str(output_path),
        ],
        cwd=Path(__file__).resolve().parents[1],
    ...
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    config_payload = json.loads(scheduler_config.read_text(encoding="utf-8"))
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_materialize"
    assert payload["read_only"] is False
    assert payload["mutated"] is True
    assert payload["materialized_config"]["written"] is True
    assert payload["materialized_config"]["enabled"] is True
    assert payload["materialized_config"]["recurring_scheduler_enabled"] is True
    assert payload["materialized_config"]["background_or_cron_enabled"] is False
    assert payload["materialized_config"]["path"] == str(scheduler_config.resolve(strict=False))
    assert payload["materialized_config"]["sha256"] == hashlib.sha256(scheduler_config.read_bytes()).hexdigest()
    ...
    assert payload["automation_authority"] == {
        ...
        "enables_unattended_default_authority": False,
        "writes_scheduler_config": True,
        "writes_enabled_config_only": True,
    }
    assert payload["quality_gate"] == {
        "pass": True,
        "decision": "ordinary_turn_default_automation_enabled_recurring_scheduler_config_materialize_green_enabled_config_written_no_background",
        "blocked_reasons": [],
    }

    red_config = tmp_path / "red-enabled-config.json"
    red_result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert not red_config.exists()




def test_dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_one_cycle_smoke_is_read_only_gate(
    tmp_path: Path,
) -> None:
    scheduler_config = tmp_path / "ordinary-turn-default-automation-recurring-scheduler.enabled.json"
    materialize_report = tmp_path / "enabled-recurring-scheduler-config-materialize.json"
    output_path = tmp_path / "enabled-recurring-scheduler-config-one-cycle-smoke.json"
    ...
    materialize_report.write_text(
        json.dumps(
            {
                "kind": "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_materialize",
                "read_only": False,
                "mutated": True,
                "default_retrieval_unchanged": True,
                "ordinary_conversation_auto_approval": False,
                "materialized_config": {
                    "path": str(scheduler_config.resolve(strict=False)),
                    "written": True,
                    "enabled": True,
                    "recurring_scheduler_enabled": True,
                    "background_or_cron_enabled": False,
                    "sha256": hashlib.sha256(scheduler_config.read_bytes()).hexdigest(),
                },
                "automation_authority": {
                    "executes_scheduler_cycle": False,
                    "executes_apply": False,
    ...
    result = subprocess.run(
        [
            ...
            "agent_memory.api.cli",
            "dogfood",
            "ordinary-turn-default-automation-enabled-recurring-scheduler-config-one-cycle-smoke",
            "--materialize-report",
            str(materialize_report),
            "--scheduler-config",
            str(scheduler_config),
    ...
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_one_cycle_smoke"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["one_cycle_gate"]["ready"] is True
    assert payload["one_cycle_gate"]["max_candidates_per_cycle"] == 1
    assert payload["one_cycle_gate"]["scheduler_cycle_executed"] is False
    assert payload["one_cycle_gate"]["apply_executed"] is False
    assert payload["one_cycle_gate"]["background_or_cron_enabled"] is False
    assert payload["one_cycle_gate"]["package_stop_required"] is True
    assert payload["one_cycle_gate"]["post_apply_verification_required_before_next_cycle"] is True
    assert payload["one_cycle_command_preview"]["dogfood_action"] == "ordinary-turn-default-automation-scheduler-one-shot"
    assert payload["one_cycle_command_preview"]["schedule_approval_phrase"] == "run-one-local-default-automation-schedule-v1"
    assert payload["one_cycle_command_preview"]["apply_approval_phrase"] == "apply-exact-ordinary-turn-default-automation-candidate-v1"
    assert payload["one_cycle_command_preview"]["requires_private_reason_placeholder"] is True
    assert payload["automation_authority"] == {
        "executes_scheduler_cycle": False,
        "executes_apply": False,
        "recurring_scheduler_enabled": True,
    ...
    unsafe_config = json.loads(scheduler_config.read_text(encoding="utf-8"))
    unsafe_config["background_or_cron_enabled"] = True
    scheduler_config.write_text(json.dumps(unsafe_config), encoding="utf-8")
    red_result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert red_payload["mutated"] is False
    assert red_payload["quality_gate"]["pass"] is False
    assert "scheduler_config_background_or_cron_enabled" in red_payload["quality_gate"]["blocked_reasons"]
...
def test_dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_one_cycle_post_run_verification_hardens_recurrence_preflight(
    tmp_path: Path,
) -> None:
    one_shot_output = tmp_path / "scheduler-one-shot.json"
    execute_output = tmp_path / "enabled-one-cycle-execute.json"
    verifier_output = tmp_path / "one-cycle-post-run-verification.json"
    env = {**os.environ, "PYTHONPATH": "src"}
    evidence_rollup = tmp_path / "ordinary-turn-default-automation-evidence-rollup.json"
    evidence_rollup.write_text(
        json.dumps(
            {
                "kind": "dogfood_ordinary_turn_default_automation_evidence_rollup",
                "quality_gate": {"pass": True, "blocked_reasons": []},
    ...
    one_shot_payload = {
        ...
        "default_retrieval_unchanged": True,
        "ordinary_conversation_auto_approval": False,
        "scheduler_status": {"quality_gate_pass": True, "ready_for_next_explicit_scheduler_cycle": True},
        "scheduler_integration": {"quality_gate_pass": True, "selected_trace_ref": "experience_trace:42"},
        "scheduler_package": {
            "quality_gate_pass": True,
            "evidence_rollup_path": str(evidence_rollup),
    ...
    one_shot_output.write_text(json.dumps(one_shot_payload), encoding="utf-8")
    execute_payload = {
        "kind": "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_one_cycle_execute",
        "read_only": False,
        "mutated": True,
        "default_retrieval_unchanged": True,
        "ordinary_conversation_auto_approval": False,
        "one_cycle_smoke": {"quality_gate_pass": True, "ready": True},
        "scheduler_one_shot": {
            "path": str(one_shot_output),
            "sha256": hashlib.sha256(one_shot_output.read_bytes()).hexdigest(),
            "quality_gate_pass": True,
            "db_approval_mode": "copy",
            "source_db_mutated": False,
            "copy_db_mutated": True,
            "scheduler_package_quality_gate_pass": True,
            "evidence_rollup_quality_gate_pass": True,
            "selected_trace_ref": "experience_trace:42",
        },
        "execution_boundary": {
            "smoke_gate_consumed": True,
            "scheduler_one_shot_executed": True,
    ...
    execute_output.write_text(json.dumps(execute_payload), encoding="utf-8")
    ...
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert json.loads(verifier_output.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_one_cycle_post_run_verification"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["post_run_verification"] == {
        "one_cycle_execute_green": True,
        "scheduler_one_shot_green": True,
        "scheduler_one_shot_hash_matches": True,
        "scheduler_package_green": True,
        "evidence_rollup_green": True,
        "source_copy_boundary_verified": True,
        "stopped_after_one_package": True,
    }
    assert payload["recurrence_install_preflight"] == {
        "ready_for_preflight_packet": True,
    ...
    assert payload["recommended_next_step"] == "build_recurrence_install_preflight_packet_without_installing_background_or_cron"

    tampered = json.loads(execute_output.read_text(encoding="utf-8"))
    tampered["automation_authority"]["background_or_recurring_schedule_enabled"] = True
    execute_output.write_text(json.dumps(tampered), encoding="utf-8")
    red_result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    red_payload = json.loads(red_result.stdout)
    assert red_payload["quality_gate"]["pass"] is False
    assert "one_cycle_execute_background_or_recurring_enabled" in red_payload["quality_gate"]["blocked_reasons"]



def test_dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_recurrence_install_preflight_requires_green_post_run_verifier(
    tmp_path: Path,
) -> None:
    post_run_report = tmp_path / "one-cycle-post-run-verification.json"
    output_path = tmp_path / "recurrence-install-preflight.json"
    env = {**os.environ, "PYTHONPATH": "src"}
    post_run_report.write_text(
        json.dumps(
            {
    ...
    result = subprocess.run(
        [
            ...
            str(post_run_report),
            "--approval-phrase",
            "preflight-recurrence-install-default-automation-scheduler-v1",
            "--cadence-policy",
            "bounded local cadence with one candidate per cycle",
            "--kill-switch-policy",
            "operator kill switch path must exist before activation",
            "--output",
            str(output_path),
        ],
        cwd=Path(__file__).resolve().parents[1],
    ...
    assert result.returncode == 0, result.stderr
    assert "bounded local cadence" not in result.stdout
    assert "operator kill switch" not in result.stdout
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_enabled_recurring_scheduler_config_recurrence_install_preflight"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["preflight_inputs"]["post_run_verification_quality_gate_pass"] is True
    assert payload["preflight_inputs"]["cadence_policy_sha256"] == hashlib.sha256(
        b"bounded local cadence with one candidate per cycle"
    ).hexdigest()
    assert payload["preflight_inputs"]["kill_switch_policy_sha256"] == hashlib.sha256(
        b"operator kill switch path must exist before activation"
    ).hexdigest()
    assert payload["recurrence_install_gate"] == {
    ...
    assert payload["recommended_next_step"] == "build_exact_approved_activation_packet_without_starting_background_or_cron"

    tampered = json.loads(post_run_report.read_text(encoding="utf-8"))
    tampered["recurrence_install_preflight"]["background_or_cron_install_allowed"] = True
    post_run_report.write_text(json.dumps(tampered), encoding="utf-8")
    red_result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    red_payload = json.loads(red_result.stdout)
    assert red_payload["quality_gate"]["pass"] is False
    assert "post_run_preflight_background_or_cron_allowed" in red_payload["quality_gate"]["blocked_reasons"]
...
def test_dogfood_ordinary_turn_default_automation_disabled_recurring_scheduler_config_contract_is_data_only(
    tmp_path: Path,
) -> None:
    readiness_report = tmp_path / "recurring-scheduler-readiness.json"
    output_path = tmp_path / "disabled-recurring-scheduler-config-contract.json"
    cadence_policy = "operator-reviewed-one-cycle-at-a-time"
    kill_switch_policy = "disable-policy-state-or-remove-scheduler-config"
    readiness_report.write_text(
        json.dumps(
            {
                ...
                "default_retrieval_unchanged": True,
                "ordinary_conversation_auto_approval": False,
                "source_report": {"quality_gate_pass": True},
                ...
                "operator_policies": {
                    ...
                    "cadence_policy_sha256": hashlib.sha256(cadence_policy.encode("utf-8")).hexdigest(),
                    "kill_switch_policy_ref": "provided_nonempty_hash_only",
                    "kill_switch_policy_sha256": hashlib.sha256(kill_switch_policy.encode("utf-8")).hexdigest(),
                },
    ...
    result = subprocess.run(
        [
            ...
            "agent_memory.api.cli",
            "dogfood",
            "ordinary-turn-default-automation-disabled-recurring-scheduler-config-contract",
            "--recurring-scheduler-readiness-report",
            str(readiness_report),
            "--policy",
            "ordinary-turn-default-automation-policy-v1",
            "--approval-phrase",
            "approve-disabled-recurring-default-automation-scheduler-config-contract-v1",
            "--output",
            str(output_path),
        ],
        cwd=Path(__file__).resolve().parents[1],
    ...
    assert result.returncode == 0, result.stderr
    assert cadence_policy not in result.stderr
    assert kill_switch_policy not in result.stderr
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_disabled_recurring_scheduler_config_contract"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["quality_gate"] == {
        "pass": True,
    ...
    assert payload["operator_policy_refs"] == {
...
def test_dogfood_ordinary_turn_default_automation_disabled_recurring_scheduler_config_materialize_writes_disabled_only(
    tmp_path: Path,
) -> None:
    validation_report = tmp_path / "disabled-recurring-scheduler-config-validation.json"
    config_path = tmp_path / "ordinary-turn-default-automation-recurring-scheduler.disabled.json"
    output_path = tmp_path / "disabled-recurring-scheduler-config-materialize.json"
    validation_report.write_text(
        json.dumps(
            {
                "kind": "dogfood_ordinary_turn_default_automation_disabled_recurring_scheduler_config_validation",
                "read_only": True,
                "mutated": False,
                "default_retrieval_unchanged": True,
                "ordinary_conversation_auto_approval": False,
    ...
    result = subprocess.run(
        [
            ...
            "--scheduler-config-output",
            str(config_path),
            "--policy",
            "ordinary-turn-default-automation-policy-v1",
            "--approval-phrase",
            "approve-materialize-disabled-recurring-default-automation-scheduler-config-v1",
            "--output",
            str(output_path),
        ],
        cwd=Path(__file__).resolve().parents[1],
    ...
    assert config_payload == {
        ...
        "policy": "ordinary-turn-default-automation-policy-v1",
        "mode": "disabled_recurring_scheduler_contract_v1",
        "contract_validation_sha256": hashlib.sha256(validation_report.read_bytes()).hexdigest(),
        "max_candidates_per_cycle": 1,
        "requires_enabled_policy_state": True,
        "requires_previous_evidence_rollup": True,
        "requires_post_apply_verification_before_next_cycle": True,
    ...
    assert payload["materialized_config"]["enabled"] is False
    assert payload["materialized_config"]["path"] == str(config_path.resolve(strict=False))
    assert payload["automation_authority"] == {
        "executes_scheduler_cycle": False,
        "executes_apply": False,
        "recurring_scheduler_enabled": False,
...
def test_dogfood_ordinary_turn_default_automation_post_apply_verification_green_stop(
    tmp_path: Path,
) -> None:
    ...
    trace = insert_experience_trace(
        ...
        event_kind="turn",
        content_sha256="c" * 64,
        summary="User prefers compact default automation summaries.",
        scope="project:agent-memory",
        session_ref="session:default-automation-post-apply",
        salience=0.9,
        user_emphasis=0.8,
        retention_policy="review",
        metadata={"ordinary_turn": True},
    ...
    apply_result = subprocess.run(
        [
            ...
            "test-operator",
            "--reason",
            "exact default automation post apply verifier setup",
            "--backup-path",
            str(backup_path),
            "--output",
            str(apply_report),
    ...
    payload = json.loads(verify_result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_post_apply_verification"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["default_retrieval_unchanged"] is True
    assert payload["ordinary_conversation_auto_approval"] is False
    assert payload["quality_gate"] == {
        "pass": True,
        "blocked_reasons": [],
        "decision": "ordinary_turn_default_automation_post_apply_verification_green_stop",
    }
    assert payload["apply_report"]["applied_count"] == 1
    ...
    assert payload["application_audit"]["policy"] == "ordinary-turn-default-automation-policy-v1"
    assert payload["application_audit"]["action"] == "apply_ordinary_turn_default_automation_preference"
    ...
    assert "exact default automation post apply verifier setup" not in verify_result.stdout


def test_dogfood_ordinary_turn_default_automation_post_apply_verification_blocks_unsafe_artifacts(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "ordinary-turn-default-automation-post-apply-blocked.db"
    backup_path = tmp_path / "missing.backup.db"
    apply_report = tmp_path / "bad-default-apply.json"
    rollback_report = tmp_path / "bad-default-rollback.json"
    initialize_database(db_path)
    apply_report.write_text(
        json.dumps(
            {
                ...
                "policy": "wrong-policy",
                "backup": {"path": str(backup_path), "sha256": "0" * 64},
                "apply": {"applied_count": 2, "trace_ref": "bad", "memory_ref": "bad"},
                "quality_gate": {"pass": False},
                "forbidden_authority": {
                    "ordinary_conversation_auto_approval": True,
                    "broad_background_apply_allowed": True,
                    "default_background_auto_approval_allowed": True,
                    "unattended_default_apply_allowed": True,
                    "default_ranking_mutated": True,
                    "collapse_delete_apply_allowed": True,
                    "telemetry_reset_apply_allowed": True,
                    "unreviewed_promotion_allowed": True,
                    "repeated_apply_without_new_approval_allowed": True,
                },
                "privacy": {"raw_content_included": True, "backup_content_included": True},
            }
        ),
        encoding="utf-8",
    )
        ...
        json.dumps(
            {
                ...
                "quality_gate": {"pass": False, "blocked_reasons": ["backup_missing"]},
                "privacy": {"raw_content_included": True, "backup_content_included": True},
            }
        ),
        encoding="utf-8",
    ...
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["quality_gate"]["decision"] == "fix_ordinary_turn_default_automation_post_apply_verification_before_next_apply"
    assert payload["quality_gate"]["pass"] is False
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["ordinary_conversation_auto_approval"] is False
    assert payload["quality_gate"]["blocked_reasons"] == [
        ...
        "apply_report_forbidden_authority_broad_background_apply_allowed_invalid",
        "apply_report_forbidden_authority_collapse_delete_apply_allowed_invalid",
        "apply_report_forbidden_authority_default_background_auto_approval_allowed_invalid",
        "apply_report_forbidden_authority_default_ranking_mutated_invalid",
        "apply_report_forbidden_authority_ordinary_conversation_auto_approval_invalid",
        "apply_report_forbidden_authority_repeated_apply_without_new_approval_allowed_invalid",
        "apply_report_forbidden_authority_telemetry_reset_apply_allowed_invalid",
        "apply_report_forbidden_authority_unattended_default_apply_allowed_invalid",
        "apply_report_forbidden_authority_unreviewed_promotion_allowed_invalid",
        "apply_report_memory_ref_invalid",
        "apply_report_ordinary_auto_approval_enabled",
        "apply_report_policy_mismatch",
        "apply_report_privacy_not_ref_safe",
        "apply_report_quality_gate_not_green",
        "apply_report_trace_ref_invalid",
        "backup_file_missing",
        "backup_sha256_mismatch",
...
def test_dogfood_ordinary_turn_default_automation_dry_run_blocks_red_policy_gate(
    ...
    db_path = tmp_path / "ordinary-turn-default-automation-dry-run-red.db"
    gate_path = tmp_path / "ordinary-turn-default-automation-policy-gate-red.json"
    initialize_database(db_path)
    _write_ordinary_turn_default_automation_policy_gate_report(
        gate_path,
        pass_gate=False,
        ready_for_opt_in_dry_run=False,
        blocked_reasons=["secret_like_ordinary_turns_present"],
    )
    insert_experience_trace(
        db_path,
    ...
    assert payload["quality_gate"] == {
        "pass": False,
        "decision": "ordinary_turn_default_automation_dry_run_not_ready_keep_default_blocked",
        "blocked_reasons": [
            "policy_gate_not_green",
            "policy_gate_not_ready_for_opt_in_dry_run",
        ],
    }
    assert payload["candidate_counts"]["selected_candidate_count"] == 0
    assert payload["policy_contract"]["default_auto_approval_enabled"] is False
...
def _write_ordinary_turn_default_automation_post_apply_verification_report(
    path: Path,
    *,
    trace_ref: str,
    memory_ref: str,
    pass_gate: bool = True,
    policy: str = "ordinary-turn-default-automation-policy-v1",
    blocked_reasons: list[str] | None = None,
) -> None:
    blocked = blocked_reasons or ([] if pass_gate else ["ordinary_turn_relation_missing"])
    path.write_text(
        json.dumps(
            {
                "kind": "dogfood_ordinary_turn_default_automation_post_apply_verification",
                "read_only": True,
                "mutated": False,
                "default_retrieval_unchanged": True,
                "ordinary_conversation_auto_approval": False,
                ...
                "application_audit": {
                    ...
                    "policy": policy,
                    "action": "apply_ordinary_turn_default_automation_preference",
                    "promoted_ref": memory_ref,
                    "backup_sha256_matches": True,
                },
                "relation_evidence": {
                    "ordinary_turn_relation_found": True,
                ...
                "quality_gate": {
                    ...
                    "decision": "ordinary_turn_default_automation_post_apply_verification_green_stop"
                    if pass_gate
                    else "fix_ordinary_turn_default_automation_post_apply_verification_before_next_apply",
                },
                "forbidden_authority": {
                    "executes_apply": False,
                    "ordinary_conversation_auto_approval": False,
...
def test_dogfood_ordinary_turn_default_automation_evidence_rollup_summarizes_repeated_green_verifiers(
    tmp_path: Path,
) -> None:
    report_one = tmp_path / "default-post-apply-one.json"
    report_two = tmp_path / "default-post-apply-two.json"
    output_path = tmp_path / "default-automation-evidence-rollup.json"
    _write_ordinary_turn_default_automation_post_apply_verification_report(
        report_one, trace_ref="experience_trace:101", memory_ref="fact:201"
    )
    _write_ordinary_turn_default_automation_post_apply_verification_report(
        report_two, trace_ref="experience_trace:102", memory_ref="fact:202"
    )
    env = {**os.environ, "PYTHONPATH": "src"}
    ...
    result = subprocess.run(
        [
            ...
            "agent_memory.api.cli",
            "dogfood",
            "ordinary-turn-default-automation-evidence-rollup",
            "--post-apply-verification-report",
            str(report_one),
            "--post-apply-verification-report",
            str(report_two),
    ...
    payload = json.loads(result.stdout)
    assert json.loads(output_path.read_text(encoding="utf-8")) == payload
    assert payload["kind"] == "dogfood_ordinary_turn_default_automation_evidence_rollup"
    assert payload["read_only"] is True
    assert payload["mutated"] is False
    assert payload["default_retrieval_unchanged"] is True
    assert payload["ordinary_conversation_auto_approval"] is False
    assert payload["evidence_summary"] == {
        ...
        "min_green_reports": 2,
        "ready_for_default_enablement_design": True,
        "apply_supported": False,
        "apply_executed": False,
        "default_auto_approval_enabled": False,
    }
...
def test_dogfood_ordinary_turn_default_automation_evidence_rollup_blocks_reused_or_red_verifiers(
    tmp_path: Path,
) -> None:
    report_one = tmp_path / "default-post-apply-one.json"
    report_two = tmp_path / "default-post-apply-two.json"
    ...
    _write_ordinary_turn_default_automation_post_apply_verification_report(
        report_two,
        trace_ref="experience_trace:101",
        memory_ref="fact:201",
        pass_gate=False,
        blocked_reasons=["ordinary_turn_relation_missing"],
    )
    env = {**os.environ, "PYTHONPATH": "src"}
    ...
    payload = json.loads(result.stdout)
    assert payload["quality_gate"]["pass"] is False
    assert payload["quality_gate"]["decision"] == "collect_more_ordinary_turn_default_automation_evidence_before_enablement_design"
    assert payload["quality_gate"]["blocked_reasons"] == [
        "green_report_count_below_minimum",
        "memory_ref_reuse_detected",
        "post_apply_verification_gate_not_green",
        "post_apply_verification_quality_reason_ordinary_turn_relation_missing",
        "trace_ref_reuse_detected",
    ]
    assert payload["evidence_summary"]["ready_for_default_enablement_design"] is False
    assert payload["forbidden_authority"]["unattended_default_apply_allowed"] is False
