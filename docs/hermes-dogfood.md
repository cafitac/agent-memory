...
For Stage G/G1 dogfood, explicit `Remember this:` / `Please remember:` turns become higher-salience `remember_intent` review traces when the message passes the conservative secret-like scan. These rows carry sanitized summaries, `retention_policy=review`, high salience/user emphasis, and metadata showing review is required and auto-approval is false. They are reviewable through the existing read-only consolidation candidate/explain commands; they do not create or approve facts/procedures/episodes automatically. Ordinary turns remain weak evidence-only traces and are not eligible for auto-approval.
