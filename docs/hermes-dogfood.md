...
`graph export-html` is a local browser view for the graph substrate. It writes a standalone, event-driven neural-style HTML file and returns a small JSON receipt with `read_only: true` and `mutated: false`. By default it uses ref-only labels and excludes raw source content, raw query text, trace summaries, prompts, transcripts, and samples. The browser view uses a Korean-localized visible UI, deterministic brain-like layout, filters/search, a dominant-hub explanation panel, render stats, and event-driven canvas redraws instead of a browser force simulation. It also exposes quality controls: `auto` caps high-DPI drawing at 1.5x for Retina-style displays, `performance` drops to DPR 1 and reduces blur/glow/labels, and `sharp` opts back into up to 2x DPR. `--include-memory-labels` is an explicit local-only opt-in for curated fact/procedure/episode labels; it still does not embed raw source/query/trace text.
...
Stage B trace work includes a manual CLI for explicit local dogfood plus default Hermes hook capture for real turns. `experience_traces` is a local substrate for bounded event fingerprints and sanitized summaries/signals; `traces record` writes only when invoked explicitly, and `traces list` emits read-only filtered JSON. Real Hermes pre-LLM turns now write metadata-only ordinary `turn` traces by default: hash/fingerprint, hashed session ref, surface/scope, safe platform/model metadata, related retrieved memory refs, low salience, and ephemeral retention. Raw prompts, raw queries, query previews, transcripts, user messages, and secret-like content are not stored or emitted. Use `--no-record-trace` on `hermes-pre-llm-hook` for a one-off/runtime opt-out.

For Stage G/G1 dogfood, explicit `Remember this:` / `Please remember:` / `기억해둬:` / `기억해줘:` turns become higher-salience `remember_intent` review traces when the message passes the conservative secret-like scan. These rows carry sanitized summaries, `retention_policy=review`, high salience/user emphasis, and metadata showing review is required and auto-approval is false. Secret-like explicit remember requests are retained only as rejected metadata-only diagnostics with no summary/raw prompt and a safe rejection reason, so operators can quality-check what was blocked without leaking the blocked text. They are reviewable through the existing read-only consolidation candidate/explain commands; they do not create or approve facts/procedures/episodes and they do not enter normal prompt retrieval by default.

`dogfood remember-intent` is the read-only G1 quality gate before any G2 auto-approval policy. It counts inspected `remember_intent` and ordinary turn traces, reports review-ready counts, safe rejection counts, scope distribution, safe samples, and unsafe sample counts. It intentionally omits raw trace metadata, raw prompts, and secret-like summaries; it does not mutate traces, memory records, relations, counters, retrieval ranking, or Hermes hook behavior.
...
Only review-ready `remember_intent` traces in the selected scope whose sanitized summary is shaped like `User prefers ...` or `I prefer ...` are eligible. The apply path defaults to stop-after-one with `--max-apply 1`, reports additional eligible traces as `deferred`, and skips traces that already have an `auto_approved_as` relation so re-runs cannot duplicate the same approval. The remember-preferences policy uses a narrow preference-topic claim slot for conflict preflight: same-topic contradictions still block, while independent preference topics can coexist and be approved across separate one-at-a-time runs. The policy blocks secret-like summaries, ordinary turns, non-matching scopes, unsupported summary shapes, and same-topic claim-slot conflicts. Successful apply writes an approved `fact` with predicate `prefers`, normal status-transition audit history, and an `auto_approved_as` relation from the trace to the fact. Use `review history`, `review explain`, and `graph inspect` to inspect or roll forward with explicit human review; do not treat this policy as broad conversation memory automation.
...
`dogfood ordinary-trace-metadata-cleanup` is the second narrow G4 cleanup surface for legacy ordinary `turn` traces that are already metadata-only (`summary: null`, `retention_policy: ephemeral`) but are missing the conservative metadata defaults `candidate_policy: evidence_only` and/or `auto_approved: false`. Preview mode is read-only and aggregate-only. Apply mode requires `--apply --actor --reason`, fills only those two metadata defaults on eligible rows, writes a hash-only audit trace, and does not print raw trace metadata, prompts, transcripts, query values, sample values, or raw reason text.

`dogfood trace-quality` is the G3d read-only quality gate for deciding whether ordinary metadata-only traces are useful enough for later consolidation work. It reports aggregate observation-to-trace coverage, empty retrieval ratio, repeated memory-ref counts, trace event-kind and retention-policy distributions, ordinary metadata-only and metadata JSON invariants, candidate-signal proxy counts, and a conservative recommendation (`continue_dogfooding`, `ready_for_more_dry_runs`, or `consider_g4_plan`). It does not print raw conversation content, raw queries, trace summaries, prompts, transcripts, API keys, or sample values, and it does not create candidates, approve memories, mutate traces/observations/activations, change ranking, or alter hook behavior.

`dogfood scheduled-dry-run` is the G3e cron-friendly bundle to run repeatedly before any G4 apply-mode plan. It emits `kind: dogfood_scheduled_dry_run`, writes the same JSON to `--output` when provided, and combines `storage_health`, `trace_quality`, `remember_intent`, and inline `memory_consolidation_background_dry_run` reports under one read-only quality gate. It is still a dry-run diagnostic: no cleanup apply mode, no ordinary-conversation auto-approval, no memory promotion, no status/relation mutation, and no default retrieval or hook config changes. A passing gate means only "write a separate G4 plan with RED tests"; failures or warnings mean continue scheduled dogfooding.
