...
def _remember_preference_forbidden_authority_payload() -> dict[str, bool]:
    return {
        "ordinary_conversation_auto_approval": False,
        "broad_background_apply": False,
        "unattended_batch_apply": False,
        "batch_apply_without_separate_verifier": False,
        "default_retrieval_ranking_mutation": False,
        "collapse_delete_apply": False,
        "telemetry_reset": False,
...
def _remember_preference_privacy_payload() -> dict[str, bool]:
    return {
        "raw_reason_included": False,
        "raw_trace_summary_included": False,
        "raw_content_included": False,
...
def _remember_preference_artifact_privacy_safe(payload: dict[str, Any]) -> bool:
    ...
    forbidden_true_keys = (
        ...
        "candidate_json_included",
        "trace_ids_included",
        "backup_contents_included",
        "sample_values_included",
    )
    return privacy.get("aggregate_or_ref_only") is True and not any(privacy.get(key) is True for key in forbidden_true_keys)


def _remember_preference_batch_graduation_readiness_payload(
    *,
    post_apply_verification_report_paths: list[Path],
    current_dry_run_report_path: Path,
    expected_policy: str,
    min_prior_verified_applies: int,
    max_batch_size: int,
    output: Path | None,
) -> dict[str, Any]:
    if expected_policy not in _REMEMBER_PREFERENCE_POLICIES:
        raise ValueError("unsupported auto-approval policy")
    blocked_reasons: list[str] = []
    if min_prior_verified_applies < 1:
        blocked_reasons.append("min_prior_verified_applies_must_be_positive")
    if max_batch_size < 2 or max_batch_size > 5:
        blocked_reasons.append("max_batch_size_outside_guardrail")

    green_verified_apply_count = 0
    verified_approved_count = 0
    scopes: set[str] = set()
    verification_artifacts: list[dict[str, Any]] = []
    for report_path in post_apply_verification_report_paths:
        ...
        if report is None:
            ...
            continue
        if report.get("kind") != "remember_preference_post_apply_verification":
            blocked_reasons.append("post_apply_verification_kind_mismatch")
        if report.get("expected_policy") != expected_policy:
        ...
        if not _remember_preference_artifact_privacy_safe(report):
        ...
        apply_summary = report.get("apply_report_summary", {}) if isinstance(report.get("apply_report_summary"), dict) else {}
        approved_count = _safe_int(apply_summary.get("approved_count"))
        if approved_count < 1:
            blocked_reasons.append("post_apply_verification_missing_approved_apply")
        if _safe_int(apply_summary.get("max_apply")) > 1:
            blocked_reasons.append("prior_apply_was_not_one_at_a_time")
        scope = apply_summary.get("scope")
        if isinstance(scope, str) and scope:
            scopes.add(scope)
        if quality_gate.get("pass") is True and approved_count >= 1:
            green_verified_apply_count += 1
            verified_approved_count += approved_count

    current_dry_run, current_artifact = _read_json_artifact_summary(current_dry_run_report_path)
    eligible_count = 0
    blocked_count = 0
    skipped_count = 0
    current_scope = None
    if current_dry_run is None:
        blocked_reasons.append("current_dry_run_report_unreadable")
    else:
        eligible_count = _safe_int(current_dry_run.get("eligible_count"))
        blocked_count = _safe_int(current_dry_run.get("blocked_count"))
        skipped_count = _safe_int(current_dry_run.get("skipped_count"))
        current_scope = current_dry_run.get("scope")
        if current_dry_run.get("kind") != "remember_preference_auto_approval_report":
            blocked_reasons.append("current_dry_run_kind_mismatch")
        if current_dry_run.get("policy") != expected_policy:
            blocked_reasons.append("current_dry_run_policy_mismatch")
        if current_dry_run.get("apply") is not False:
            blocked_reasons.append("current_dry_run_not_dry_run")
        if current_dry_run.get("read_only") is not True or current_dry_run.get("mutated") is not False:
            blocked_reasons.append("current_dry_run_not_read_only")
        if current_dry_run.get("default_retrieval_unchanged") is not True:
            blocked_reasons.append("current_dry_run_default_retrieval_changed")
        if blocked_count != 0:
            blocked_reasons.append("current_dry_run_has_blocked_candidates")
        if eligible_count < 1:
            blocked_reasons.append("current_dry_run_has_no_eligible_candidates")
        if current_scope:
            scopes.add(str(current_scope))

    if green_verified_apply_count < min_prior_verified_applies:
        blocked_reasons.append("prior_verified_apply_count_below_minimum")
    if len(scopes) > 1:
        blocked_reasons.append("mixed_scope_reports_require_separate_packets")
    ...
    payload = {
        ...
        "default_retrieval_unchanged": True,
        "expected_policy": expected_policy,
        "scope": next(iter(scopes)) if len(scopes) == 1 else current_scope,
        "min_prior_verified_applies": min_prior_verified_applies,
        "max_batch_size": max_batch_size,
        "prior_verification_summary": {
            "report_count": len(post_apply_verification_report_paths),
            "green_verified_apply_count": green_verified_apply_count,
            "verified_approved_count": verified_approved_count,
            "one_at_a_time_prior_apply_required": True,
            "artifacts": verification_artifacts,
        },
        "current_dry_run_summary": {
            "artifact": current_artifact,
            "eligible_count": eligible_count,
            "blocked_count": blocked_count,
            "skipped_count": skipped_count,
            "selected_preview_count": min(eligible_count, max_batch_size),
        },
        "quality_gate": {
            "pass": green,
            "decision": "remember_preference_bounded_batch_operator_packet_ready"
            if green
            else "fix_remember_preference_batch_graduation_before_operator_packet",
            "blocked_reasons": sorted(set(blocked_reasons)),
        },
        ...
        "next_step": "If green, build a manual-only operator packet; do not run unattended batch apply from this report.",
    }
    _write_json_report(output, payload)
    return payload
...
def _remember_preference_bounded_batch_operator_packet_payload(
    *,
    ...
    blocked_reasons: list[str] = []
    if not scope:
        blocked_reasons.append("scope_required")
    if not actor:
        blocked_reasons.append("actor_required")
    if max_apply < 2 or max_apply > 5:
        blocked_reasons.append("max_apply_outside_manual_packet_guardrail")

    graduation, graduation_artifact = _read_json_artifact_summary(graduation_readiness_report_path)
    if graduation is None:
        blocked_reasons.append("graduation_readiness_report_unreadable")
    else:
        if graduation.get("kind") != "remember_preference_batch_graduation_readiness":
            blocked_reasons.append("graduation_readiness_kind_mismatch")
        if graduation.get("expected_policy") != expected_policy:
            blocked_reasons.append("graduation_readiness_policy_mismatch")
        if graduation.get("scope") != scope:
            blocked_reasons.append("graduation_readiness_scope_mismatch")
        quality_gate = graduation.get("quality_gate", {}) if isinstance(graduation.get("quality_gate"), dict) else {}
        if quality_gate.get("pass") is not True:
            blocked_reasons.append("graduation_readiness_gate_not_green")
        if graduation.get("read_only") is not True or graduation.get("mutated") is not False:
            blocked_reasons.append("graduation_readiness_not_read_only")
        if graduation.get("default_retrieval_unchanged") is not True:
            blocked_reasons.append("graduation_readiness_default_retrieval_changed")
        if max_apply > _safe_int(graduation.get("max_batch_size"), default=max_apply):
            blocked_reasons.append("max_apply_exceeds_graduation_batch_size")
    ...
    if current_dry_run is None:
        ...
        if current_dry_run.get("policy") != expected_policy:
            blocked_reasons.append("current_dry_run_policy_mismatch")
        if current_dry_run.get("scope") != scope:
            blocked_reasons.append("current_dry_run_scope_mismatch")
        if current_dry_run.get("apply") is not False:
            blocked_reasons.append("current_dry_run_not_dry_run")
        if current_dry_run.get("read_only") is not True or current_dry_run.get("mutated") is not False:
            blocked_reasons.append("current_dry_run_not_read_only")
    ...
    selected_preview_count = min(eligible_count, max_apply)
    green = not blocked_reasons
    apply_command = (
        "agent-memory consolidation auto-approve remember-preferences <DB> "
        f"--policy {expected_policy} --scope {scope} --apply --actor {actor} "
        "--reason <PRIVATE_REVIEW_REASON> "
        f"--max-apply {max_apply} --limit 200 > <APPLY_REPORT>"
    )
    verify_command = (
        "agent-memory consolidation auto-approve remember-preferences-post-apply-verification "
        "--apply-report <APPLY_REPORT> --post-dry-run-report <POST_DRY_RUN_REPORT> "
        f"--expected-policy {expected_policy} --max-approved {max_apply} --output <VERIFY_REPORT>"
    )
    payload = {
        ...
        "scope": scope,
        "actor": actor,
        "graduation_readiness_artifact": graduation_artifact,
        "current_dry_run_artifact": current_artifact,
        "candidate_inventory": {
            "eligible_count": eligible_count,
            "selected_preview_count": selected_preview_count,
            "max_apply": max_apply,
            "candidate_json_included": False,
            "trace_ids_included": False,
        },
        "bounded_batch_apply_command_template": apply_command,
        "post_apply_verification_command_template": verify_command,
        "quality_gate": {
            "pass": green,
            "decision": "remember_preference_bounded_batch_packet_ready_for_exact_manual_apply"
            if green
            else "fix_remember_preference_bounded_batch_packet_inputs",
            "blocked_reasons": sorted(set(blocked_reasons)),
        },
        "runbook_contract": {
...
def _load_ordinary_turn_default_automation_dry_run_for_apply(report_path: Path, *, trace_ref: str) -> dict[str, Any]:
    payload, base = _read_json_artifact_summary(report_path)
    raw = payload or {}
    quality_gate = raw.get("quality_gate") if isinstance(raw.get("quality_gate"), dict) else {}
    policy_contract = raw.get("policy_contract") if isinstance(raw.get("policy_contract"), dict) else {}
    candidate_counts = raw.get("candidate_counts") if isinstance(raw.get("candidate_counts"), dict) else {}
    privacy = raw.get("privacy") if isinstance(raw.get("privacy"), dict) else {}
    ...
    candidate_preview = raw.get("candidate_preview") if isinstance(raw.get("candidate_preview"), list) else []
    blocked_reasons: list[str] = []
    if not base["provided"] or base["error"] is not None or payload is None:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_unreadable")
    if base["kind"] != "dogfood_ordinary_turn_default_automation_dry_run":
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_kind_mismatch")
    if base["read_only"] is not True or base["mutated"] is not False:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_not_read_only")
    if base["default_retrieval_unchanged"] is not True:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_default_retrieval_changed")
    if raw.get("ordinary_conversation_auto_approval") is not False:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_auto_approval_not_blocked")
    if quality_gate.get("pass") is not True:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_not_green")
    if policy_contract.get("policy") != _ORDINARY_TURN_DEFAULT_AUTOMATION_POLICY:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_policy_mismatch")
    if policy_contract.get("ready_for_opt_in_dry_run") is not True:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_not_ready")
    for key in (
        "default_auto_approval_enabled",
        "default_background_auto_approval_allowed",
        "unattended_default_apply_allowed",
        ...
        if policy_contract.get(key) is not False:
            blocked_reasons.append(f"ordinary_turn_default_automation_dry_run_{key}_not_blocked")
    if policy_contract.get("requires_exact_apply_review") is not True:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_missing_exact_review_requirement")
    if policy_contract.get("requires_backup_before_apply") is not True:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_missing_backup_requirement")
    if policy_contract.get("requires_post_apply_verification") is not True:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_missing_post_apply_requirement")
    if policy_contract.get("requires_rollback_replay") is not True:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_missing_rollback_requirement")
    if _safe_int(candidate_counts.get("selected_candidate_count")) != 1:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_selected_candidate_count_not_one")
    if candidate_refs != [trace_ref]:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_trace_ref_mismatch")
    matching_preview = [item for item in candidate_preview if isinstance(item, dict) and item.get("trace_ref") == trace_ref]
    if len(matching_preview) != 1:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_candidate_preview_mismatch")
    elif matching_preview[0].get("memory_shape") != "preference" or matching_preview[0].get("requires_exact_apply_review") is not True:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_candidate_preview_contract_invalid")
    if not _privacy_flags_are_ref_safe(privacy) or privacy.get("raw_reason_included") is True or privacy.get("backup_content_included") is True:
        blocked_reasons.append("ordinary_turn_default_automation_dry_run_privacy_not_ref_safe")
    for key in (
        "executes_apply",
        ...
        "unattended_batch_apply_allowed",
    ):
        if forbidden.get(key) is not False:
            blocked_reasons.append(f"ordinary_turn_default_automation_dry_run_forbidden_authority_{key}_invalid")
...
def _dogfood_ordinary_turn_default_automation_apply_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.policy != _ORDINARY_TURN_DEFAULT_AUTOMATION_POLICY:
        raise ValueError("ordinary_turn_default_automation_apply_policy_mismatch")
    if args.approval_phrase != approval_phrase:
        raise ValueError("ordinary_turn_default_automation_apply_approval_phrase_mismatch")
    if not args.actor.strip():
        raise ValueError("dogfood ordinary-turn-default-automation-apply --actor is required")
    if not args.reason.strip():
        raise ValueError("dogfood ordinary-turn-default-automation-apply --reason is required")
    ...
    with connect(db_path) as connection:
        ...
        if _metadata_bool(metadata.get("ordinary_turn")) is False:
            raise ValueError("experience_trace_not_marked_ordinary_turn")
        ...
        if proposed_object is None or classification_reason != "ordinary_preference":
            raise ValueError("ordinary_turn_default_automation_apply_supports_preference_shape_only")
        conflict_preflight = _remember_preference_conflict_preflight(
            db_path,
            object_ref_or_value=proposed_object,
            scope=row["scope"],
        ...
        if conflict_preflight["result"] == "blocked":
            raise ValueError("ordinary_turn_default_automation_preference_conflict_blocked")
        for relation in list_relations_for_node(db_path, node_ref=trace_ref):
            if relation.from_ref == trace_ref and relation.relation_type in {
                "ordinary_turn_default_automation_approved_as",
    ...
    reason_sha256 = hashlib.sha256(args.reason.strip().encode("utf-8")).hexdigest()
    candidate_id = f"ordinary-turn-default-automation-{trace_id}"
    source = ingest_source_text(
        db_path,
        source_type="ordinary_turn_default_automation_trace",
        content=row["summary"] or "ordinary turn default automation preference",
        adapter="agent-memory-ordinary-turn-default-automation-apply",
        external_ref=trace_ref,
        metadata={
            "trace_id": trace_id,
            "policy": args.policy,
            "dry_run_report_sha256": dry_run["sha256"],
            "sanitized": True,
        },
    )
    fact = create_candidate_fact(
    ...
    with sqlite3.connect(db_path) as connection:
        ...
        connection.execute(
            ...
            (
                candidate_id,
                "ordinary_turn_default_automation_preference",
    ...
    payload = {
        ...
        "recommended_next_step": "run_rollback_replay_and_add_default_automation_post_apply_verification_before_any_default_enablement",
    }
    _write_json_report(args.output, payload)
    return payload
...
def _load_ordinary_turn_eval_window_summary(report_path: Path) -> dict[str, Any]:
    raw_text = report_path.read_text(encoding="utf-8")
    raw = json.loads(raw_text)
    if not isinstance(raw, dict):
        raise ValueError("ordinary_turn_eval_window_summary_not_json_object")
    report_counts = raw.get("report_counts") if isinstance(raw.get("report_counts"), dict) else {}
    labeled_window = raw.get("labeled_window") if isinstance(raw.get("labeled_window"), dict) else {}
    quality_gate = raw.get("quality_gate") if isinstance(raw.get("quality_gate"), dict) else {}
    privacy = raw.get("privacy") if isinstance(raw.get("privacy"), dict) else {}
    return {
        "report_path": str(report_path),
        ...
        "default_retrieval_unchanged": raw.get("default_retrieval_unchanged"),
        "ordinary_conversation_auto_approval": raw.get("ordinary_conversation_auto_approval"),
        "window_policy": raw.get("window_policy"),
        "quality_gate_pass": bool(quality_gate.get("pass")),
        "quality_gate_blocked_reasons": [str(reason) for reason in quality_gate.get("blocked_reasons", [])],
        "report_count": int(report_counts.get("report_count", 0) or 0),
        "quality_gate_pass_count": int(report_counts.get("quality_gate_pass_count", 0) or 0),
        "labeled_ordinary_turn_total": int(labeled_window.get("labeled_ordinary_turn_total", 0) or 0),
        "precision_percent_min": int(labeled_window.get("precision_percent_min", 0) or 0),
        "false_positive_total": int(labeled_window.get("false_positive_total", 0) or 0),
        "false_negative_total": int(labeled_window.get("false_negative_total", 0) or 0),
        "privacy": privacy,
    }


def _dogfood_ordinary_turn_inferred_approval_readiness_payload(args: argparse.Namespace) -> dict[str, Any]:
    if args.min_report_count < 1:
        raise ValueError("dogfood ordinary-turn-inferred-approval-readiness --min-report-count must be >= 1")
    if args.min_labeled_total < 1:
        raise ValueError("dogfood ordinary-turn-inferred-approval-readiness --min-labeled-total must be >= 1")
    if not 0 <= args.min_precision_percent <= 100:
        raise ValueError("dogfood ordinary-turn-inferred-approval-readiness --min-precision-percent must be between 0 and 100")
    summary = _load_ordinary_turn_eval_window_summary(args.window_summary.expanduser().resolve(strict=False))
    blocked_reasons: list[str] = []
    if summary["kind"] != "dogfood_ordinary_turn_eval_window_summary":
        blocked_reasons.append("window_summary_kind_mismatch")
    if summary["read_only"] is not True or summary["mutated"] is not False:
        blocked_reasons.append("window_summary_not_read_only")
    if summary["default_retrieval_unchanged"] is not True:
        blocked_reasons.append("window_summary_default_retrieval_changed")
    if summary["ordinary_conversation_auto_approval"] is not False:
        blocked_reasons.append("window_summary_auto_approval_not_blocked")
    if summary["window_policy"] != "ordinary-turn-repeated-eval-window-v1":
        blocked_reasons.append("window_summary_policy_mismatch")
    if not summary["quality_gate_pass"]:
        blocked_reasons.append("window_summary_quality_gate_not_green")
    if summary["report_count"] < args.min_report_count:
        blocked_reasons.append("window_report_count_below_minimum")
    if summary["quality_gate_pass_count"] < summary["report_count"]:
        blocked_reasons.append("window_report_quality_gate_pass_count_mismatch")
    if summary["labeled_ordinary_turn_total"] < args.min_labeled_total:
        blocked_reasons.append("labeled_ordinary_turn_total_below_minimum")
    if summary["precision_percent_min"] < args.min_precision_percent:
        blocked_reasons.append("precision_below_minimum")
    if summary["false_positive_total"]:
        blocked_reasons.append("false_positive_predictions_present")
    if summary["false_negative_total"]:
        blocked_reasons.append("false_negative_predictions_present")
    privacy = summary["privacy"]
    if (
        privacy.get("raw_trace_summary_included")
        ...
        blocked_reasons.append("window_summary_privacy_not_safe")
    blocked_reasons.extend(summary["quality_gate_blocked_reasons"])
    blocked_unique = sorted(set(blocked_reasons))
    ready_for_design = not blocked_unique
    payload = {
        ...
        "window_evidence": {
            "window_summary_path": summary["report_path"],
            "window_summary_sha256": summary["report_sha256"],
            "usable_for_readiness": ready_for_design,
            "report_count": summary["report_count"],
            "quality_gate_pass_count": summary["quality_gate_pass_count"],
            "labeled_ordinary_turn_total": summary["labeled_ordinary_turn_total"],
            "precision_percent_min": summary["precision_percent_min"],
            "false_positive_total": summary["false_positive_total"],
            "false_negative_total": summary["false_negative_total"],
        },
        "quality_gate": {
            "pass": ready_for_design,
            "decision": "ordinary_turn_inferred_approval_ready_for_separate_exact_apply_design"
            if ready_for_design
            else "ordinary_turn_inferred_approval_not_ready_keep_auto_approval_blocked",
            "blocked_reasons": blocked_unique,
        },
        ...
        "recommended_next_step": "design_separate_exact_approval_apply_corridor_keep_ordinary_auto_approval_blocked",
    }
    _write_json_report(args.output, payload)
    return payload
...
def _dogfood_lifecycle_post_apply_verification_payload(args: argparse.Namespace) -> dict[str, Any]:
    apply_payload, apply_base = _read_json_artifact_summary(args.apply_report)
    readiness_payload, readiness_base = _read_json_artifact_summary(args.readiness_report)
    rollback_payload, rollback_base = _read_json_artifact_summary(args.rollback_replay_report)
    audit_payload, audit_base = _read_json_artifact_summary(args.application_audit_report)
    apply_payload = apply_payload or {}
    ...
    applied = apply_payload.get("applied") if isinstance(apply_payload.get("applied"), list) else []
    applied_count = len(applied)
    if applied_count < 1:
        blocked_reasons.append("apply_report_has_no_applied_candidates")
    if applied_count > args.max_applied:
        blocked_reasons.append("apply_report_exceeds_max_applied")
    if apply_payload.get("policy") != args.expected_policy:
        blocked_reasons.append("apply_report_policy_mismatch")
    ...
    readiness_counts = readiness_payload.get("candidate_counts", {}) if isinstance(readiness_payload.get("candidate_counts"), dict) else {}
    approved_remaining = sum(_safe_int(counts.get("approved", 0)) for counts in readiness_counts.values() if isinstance(counts, dict))
    no_ready_apply_after_stop = readiness_quality.get("decision") == "no_exact_lifecycle_apply_candidates_ready" and approved_remaining == 0
    if not no_ready_apply_after_stop:
        blocked_reasons.append("readiness_report_has_remaining_ready_apply_candidates")
    ...
    if not _privacy_flags_are_ref_safe(readiness_privacy):
        blocked_reasons.append("readiness_report_privacy_not_ref_safe")
    ...
    blocked_unique = sorted(set(blocked_reasons))
    payload = {
        "kind": "dogfood_lifecycle_post_apply_verification",
        "read_only": True,
        "mutated": False,
        "default_retrieval_unchanged": True,
        "ordinary_conversation_auto_approval": False,
        ...
        "readiness_report": {
            **readiness_base,
            "no_ready_apply_after_stop": no_ready_apply_after_stop,
            "approved_remaining_count": approved_remaining,
        },
        "rollback_replay_report": {
            **rollback_base,
            "pass": rollback_pass,
        ...
        "quality_gate": {
            "pass": not blocked_unique,
            "blocked_reasons": blocked_unique,
            "decision": "lifecycle_post_apply_verification_green_for_one_candidate_stop" if not blocked_unique else "fix_lifecycle_post_apply_verification_before_next_apply",
        },
        "forbidden_authority": {
            "executes_apply": False,
            "broad_background_apply_allowed": False,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    consolidation_auto_batch_graduation_parser = consolidation_auto_subparsers.add_parser(
        "remember-preferences-batch-graduation-readiness",
        help="Read-only gate summarizing repeated one-at-a-time remember-preferences proofs before any batch packet.",
    )
    consolidation_auto_batch_graduation_parser.add_argument(
    ...
    consolidation_auto_batch_graduation_parser.add_argument("--current-dry-run-report", type=Path, required=True)
    consolidation_auto_batch_graduation_parser.add_argument(
        "--expected-policy",
        required=True,
        choices=sorted(_REMEMBER_PREFERENCE_POLICIES),
    )
    consolidation_auto_batch_graduation_parser.add_argument("--min-prior-verified-applies", type=int, default=3)
    consolidation_auto_batch_graduation_parser.add_argument("--max-batch-size", type=int, default=2)
    consolidation_auto_batch_graduation_parser.add_argument("--output", type=Path)
    consolidation_auto_batch_packet_parser = consolidation_auto_subparsers.add_parser(
        "remember-preferences-bounded-batch-operator-packet",
        help="Build a read-only manual operator packet for a bounded remember-preferences batch apply.",
    )
    consolidation_auto_batch_packet_parser.add_argument("--graduation-readiness-report", type=Path, required=True)
    consolidation_auto_batch_packet_parser.add_argument("--current-dry-run-report", type=Path, required=True)
    consolidation_auto_batch_packet_parser.add_argument(
        "--expected-policy",
        required=True,
        choices=sorted(_REMEMBER_PREFERENCE_POLICIES),
    )
    consolidation_auto_batch_packet_parser.add_argument("--scope", required=True)
    ...
    dogfood_lifecycle_post_apply_verification_parser.add_argument("--apply-report", type=Path, required=True)
    dogfood_lifecycle_post_apply_verification_parser.add_argument("--readiness-report", type=Path, required=True)
    dogfood_lifecycle_post_apply_verification_parser.add_argument("--rollback-replay-report", type=Path, required=True)
    dogfood_lifecycle_post_apply_verification_parser.add_argument("--application-audit-report", type=Path, required=True)
    ...
    dogfood_ordinary_turn_default_automation_apply_parser = dogfood_subparsers.add_parser(
        "ordinary-turn-default-automation-apply",
        help="Apply one exact reviewed candidate from the default automation dry-run corridor; ordinary auto-approval stays blocked.",
    )
    ...
    dogfood_ordinary_turn_inferred_approval_readiness_parser = dogfood_subparsers.add_parser(
        "ordinary-turn-inferred-approval-readiness",
        help="Summarize strict ordinary-turn eval windows into a read-only inferred-approval readiness gate without apply authority.",
    )
    dogfood_ordinary_turn_inferred_approval_readiness_parser.add_argument("--window-summary", type=Path, required=True)
    dogfood_ordinary_turn_inferred_approval_readiness_parser.add_argument("--min-report-count", type=int, default=2)
        ...
        if args.consolidation_action == "auto-approve":
            if args.auto_approval_policy_kind == "remember-preferences-post-apply-verification":
                ...
                if payload["quality_gate"]["pass"] is not True:
                    sys.exit(1)
                return
            if args.auto_approval_policy_kind == "remember-preferences-batch-graduation-readiness":
                payload = _remember_preference_batch_graduation_readiness_payload(
                    post_apply_verification_report_paths=args.post_apply_verification_report,
                    current_dry_run_report_path=args.current_dry_run_report,
                    expected_policy=args.expected_policy,
                    min_prior_verified_applies=args.min_prior_verified_applies,
                ...
                if payload["quality_gate"]["pass"] is not True:
                    sys.exit(1)
                return
            if args.auto_approval_policy_kind == "remember-preferences-bounded-batch-operator-packet":
                payload = _remember_preference_bounded_batch_operator_packet_payload(
                    graduation_readiness_report_path=args.graduation_readiness_report,
                    current_dry_run_report_path=args.current_dry_run_report,
                    expected_policy=args.expected_policy,
                    scope=args.scope,
                    actor=args.actor,
