...
Last updated: 2026-05-09 06:49 KST
...
agent-memory is currently verified through `v0.1.116`: the first narrow G4a cleanup mutation has named-policy, rollback-manifest/private-artifact, disposable cleanup preflight, restore dry-run, source DB binding, artifact-integrity checks, a blocked restore apply contract checkpoint, disposable restore rehearsal, aggregate-only restore audit preview, a blocked audit write dry-run, a blocked audit-write apply contract, a read-only audit-write preflight gate, duplicate/conflict fail-closed reporting, a dry-run audit row materialization contract, a single-row audit-write policy packet, an approval-token-missing negative gate, and an approval-token flag dry-run parser. GitHub Release, npm, and PyPI all report `v0.1.116`. The live Hermes `default`/`personal-oss` plus `earlypay` hook runtimes were upgraded to `/Users/reddit/.agent-memory/runtime/v0.1.116/.venv/bin/agent-memory`; installed-runtime QA passed with report `/Users/reddit/.agent-memory/reports/v0.1.116-runtime-qa-20260508T212306`.

Storage/privacy cleanup remains clean: legacy `retrieval_observations.query_preview` rows are expected to stay at 0, restore artifacts remain private/local because they contain raw query previews, and broad G4 consolidation apply mode remains blocked. The current branch adds only an invalid-token validator negative contract inside the already-blocked audit write apply contract; it keeps tokens hash-only, marks supplied tokens invalid-or-unverified, and still does not validate approval successfully, write audit rows, run live restore, or mutate the live DB.
...
Current slice: start from `v0.1.116` validated `main` on branch `g4/query-preview-cleanup-restore-audit-write-approval-token-validator-negative`. Add an invalid-token validator negative contract under `dogfood query-preview-cleanup-restore --apply -> restore_apply_contract.audit_preview.write_dry_run.apply_contract.single_row_apply_policy_packet` so a supplied token is still hash-only and explicitly blocked as invalid or unverified before any successful approval validation path exists. It must report `approval_token_required=true`, `approval_token_present=true`, `approval_token_validated=false`, `approval_token_validation_status=invalid_or_unverified`, `write_blocked_by_missing_approval=false`, `write_blocked_by_unvalidated_approval=true`, `write_blocked_by_invalid_approval=true`, and `restore_audit_write_approval_token_invalid` while still returning `would_insert=false` and `write_allowed=false`. Without the flag, the v0.1.115 missing-token negative gate remains in force.

Why this is the best next move: v0.1.116 freezes the token-present parser while keeping approval unvalidated. Before any audit row write or live restore, the invalid/unverified token path should be explicit and testable so future validator work cannot accidentally treat any supplied token as sufficient. Broad consolidation apply mode remains blocked — DO NOT enable broad G4 apply mode.
...
- Current feature branch for this slice: `g4/query-preview-cleanup-restore-audit-write-approval-token-validator-negative`.
- Latest merged G4a hardening PR: #242 `feat: add restore audit approval token parser`.
- Latest merged release-sync PR: #243 `chore: release v0.1.116 [skip release]`.
- Latest completed release: `v0.1.116`.
