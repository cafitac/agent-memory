...
- Rechecked the live personal DB and scheduler blocker path from the current checkout. Source is `develop` at `f2b5c03 Expose scheduled decay evidence candidates`; `origin/develop` matches before this local slice. Tracked working tree now has a new read-only human-classification packet slice plus docs/tests; unrelated untracked harness/scratch dirs remain (`.agent-learner/`, `.claude/`, `.dev/kb/retrieval-eval-m1-implementation-plan.md`, `.omc/`, `.worktrees/`).
- Fresh live artifacts were written as `/tmp/agent-memory-decay-risk-next-check.json`, `/tmp/agent-memory-scheduled-dry-run-next-check.json`, `/tmp/agent-memory-scheduled-blocker-resolution-next-check.json`, and `/tmp/agent-memory-scheduled-evidence-blocker-packet-next-check.json`.
- Fresh `activations decay-risk-report` over `/Users/reddit/.agent-memory/memory.db` with `--limit 200 --top 20` now reports `activation_count=200`, `candidate_count=8`, `max_score=0.5333`, and hint counts `collect_more_activation_evidence_before_decay_action=2` plus `monitor_only_no_mutation=6`.
- The hard evidence-collection refs are now `fact:5` and `fact:6`. `fact:5` remains approved/connected with score `0.5333` and recent-window activation_count `1`. `fact:6` re-entered as approved/connected with score `0.3667` and recent-window activation_count `2`. Both are evidence-collection/manual-classification candidates only; do not delete, collapse, deprecate, lower authority, or change ranking from decay score alone.
- Read-only `review explain/history` plus `graph inspect` for `fact:6` wrote `/tmp/agent-memory-fact6-explain-next.txt`, `/tmp/agent-memory-fact6-history-next.txt`, and `/tmp/agent-memory-fact6-graph-next.json`. It is approved, visible in default retrieval, project-scoped, and graph-connected to its approval trace only. No mutation was performed.
- Fresh `dogfood scheduled-dry-run` keeps storage healthy, trace recommendation `consider_g4_plan`, background warnings absent, and quality red only on `decay_risk_above_threshold`.
