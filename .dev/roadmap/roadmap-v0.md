...
- [>] PR G5-brainlike-consolidation-runway: Turn healthy traces into reviewed human-memory-like consolidation flow
  ...
  - Current status as of 2026-05-17 10:36 KST: approximately 99.97-99.98% toward the scoped human-brain-like local-memory lifecycle. Completed and live/copy/source-verified pieces include repeated one-at-a-time lifecycle reinforcement, recurrent reinforcement, explicit remember-intent evidence, topic-aware low-risk preference auto-approval, duplicate guards, remember-preferences post-apply verifier, explicit safe queue drain, read-only bounded-batch graduation/operator packet, ordinary-turn classifier/eval, raw-text-free label packets, exact-ref label updates, repeated eval-window summaries, metadata-only ordinary-turn memory hints, read-only inferred approval readiness over strict repeated windows, exact-approval ordinary-turn inferred preference apply with backup/audit/rollback replay on a copy DB, ordinary-turn inferred post-apply verification, repeated inferred evidence rollup, read-only broader automation readiness, exact read-only default-automation policy gate, and read-only/ref-safe default-automation dry-run over the narrowest non-secret preference-shaped ordinary-turn candidates. Still blocked until separately implemented/proven gates exist: exact-reviewed one-candidate default-automation smoke/apply corridor, broad/background ordinary-turn auto-approval, broad/background apply, unattended batch apply, default-ranking automatic rollout, collapse/delete, telemetry reset, and unreviewed promotion.
