...
- Rechecked the live `agent-memory` path from current `develop` at `f2b5c03 Expose scheduled decay evidence candidates`; `origin/develop` matched before this local slice. Tracked working tree now includes a read-only scheduled evidence blocker packet slice plus docs/tests.
- Fresh live decay-risk report over `/Users/reddit/.agent-memory/memory.db` with `--limit 200 --top 20` wrote `/tmp/agent-memory-decay-risk-next-check.json`: `activation_count=200`, `decay_risk_candidate_count=8`, `max_score=0.5333`, resolution hints `collect_more_activation_evidence_before_decay_action=2` and `monitor_only_no_mutation=6`.
- The hard evidence-collection refs are now `fact:5` and `fact:6`. `fact:5` remains approved/connected with low recent activation evidence (`activation_count=1`, score `0.5333`). `fact:6` re-entered as approved/connected with low recent activation evidence (`activation_count=2`, score `0.3667`). Treat both as evidence-collection/manual-classification candidates only; do not delete, collapse, deprecate, lower authority, or mutate ranking from decay score alone.
- Read-only inspection for `fact:6` wrote `/tmp/agent-memory-fact6-explain-next.txt`, `/tmp/agent-memory-fact6-history-next.txt`, and `/tmp/agent-memory-fact6-graph-next.json`. The ref is approved, visible in default retrieval, project-scoped, and connected to its approval trace only. No memory status, retrieval ranking, relation, or automation authority was changed.
- Fresh scheduled dry-run artifact: `/tmp/agent-memory-scheduled-dry-run-next-check.json`. It keeps storage healthy, trace recommendation `consider_g4_plan`, background warnings absent, and only `decay_risk_above_threshold` as the blocker.
- Fresh scheduled blocker-resolution artifact: `/tmp/agent-memory-scheduled-blocker-resolution-next-check.json`. It leaves decay red: `resolution=evidence_collection_candidates_still_block`, `operator_severity=hard_blocker`, `candidate_count=8`, `evidence_collection_candidate_count=2`, `monitor_only_candidate_count=6`, `bounded_partial_automation_allowed=false`, `broad_g4_apply_allowed=false`.
...
1. Finish the verification ladder for this local read-only packet slice: full suite, `git diff --check`, then commit/push only tracked source/test/doc changes and watch CI.
2. Continue normal-turn dogfood and re-run decay-risk, scheduled-dry-run, scheduled-blocker-resolution, and scheduled-evidence-blocker-packet; only consider bounded partial automation green when evidence blockers are resolved/classified through a reviewed path and all storage/privacy/linkage/background/trace checks remain clean.
3. If `fact:5`/`fact:6` continue to stay low-activation despite being approved preferences, add a separate exact human classification validation artifact over the packet. That later surface must still not mutate memory status, retrieval ranking, or default/background authority.
