...
## Current G4a safety hardening: restore audit write approval-token negative gate

`dogfood query-preview-cleanup-restore --apply` remains read-only and blocked. After v0.1.114 locked the single-row audit-write policy packet, the current slice adds only an approval-token-missing negative gate for that future audit write path: approval token required, approval token absent, approval token hash null, write blocked by missing approval, and a dedicated `restore_audit_write_approval_token_missing` blocked reason. The gate must still report `requires_explicit_operator_approval=true`, `would_insert=false`, and `write_allowed=false`; it exists to freeze the missing-token fail-closed boundary before any approval-token flag or write is considered, not to enable audit rows. Live restore and audit row writes remain unavailable until a separate explicit policy slice, and broad mutation remains blocked — DO NOT enable broad G4 apply mode.
