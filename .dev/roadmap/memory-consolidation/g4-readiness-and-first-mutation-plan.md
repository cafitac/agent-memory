...
## Historical G4a safety hardening: restore audit write approval-token hash-match still-blocked contract

This was the pre-v0.1.122 checkpoint: `dogfood query-preview-cleanup-restore --apply` remained read-only and blocked while the project froze matching-token/expected-hash behavior before approval success or writes. Current work has moved past this checkpoint: v0.1.122 validates matching approval tokens, and the active narrow safety corridor may write only a metadata-only restore audit row while live restore and broad mutation remain blocked — DO NOT enable broad G4 apply mode from this historical plan.
