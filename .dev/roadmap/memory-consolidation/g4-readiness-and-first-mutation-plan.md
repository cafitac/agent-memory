...
## Current G4a safety hardening: restore audit write preflight gate

`dogfood query-preview-cleanup-restore --apply` remains read-only and blocked. After v0.1.110 locked the blocked audit-write apply contract, the current slice adds only a deterministic preflight gate for that future audit write path: exact policy, actor presence, reason-hash continuity, source DB match, artifact integrity, disposable restore rehearsal pass, deterministic content/metadata hash match, duplicate audit event absence, and privacy checks that keep raw `query_preview`, raw reason, sample values, tokens, and API-key-like strings out of stdout/metadata. The preflight may report `passed=true` for prerequisites, but it must still report `write_allowed=false`, `would_insert=false`, and no `experience_traces` insert. Live restore and audit row writes remain unavailable until a separate explicit policy slice.
