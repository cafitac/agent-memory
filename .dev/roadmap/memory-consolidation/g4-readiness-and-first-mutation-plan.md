...
## Current G4a safety hardening: restore audit write expected approval hash flag parser

`dogfood query-preview-cleanup-restore --apply` remains read-only and blocked. After v0.1.118 locked the expected-hash-missing gate, the current slice adds only an expected approval hash parser for the future audit write path: `--approval-token-expected-sha256` is accepted, normalized to lower-case, and exposed as `approval_token_expected_sha256` plus `approval_token_expected_sha256_fingerprint_sha256`. A supplied token is still never printed raw and remains represented only by `approval_token_sha256`; even when the token hash and expected hash are both supplied, approval remains `approval_token_validated=false` with `approval_token_validation_status=validation_not_implemented`, `write_blocked_by_unvalidated_approval=true`, `write_blocked_by_invalid_approval=true`, `write_blocked_by_missing_expected_approval_hash=false`, and `restore_audit_write_approval_token_validation_not_implemented`. Without the expected-hash flag, the expected-hash-missing fail-closed contract remains in force. All paths must still report `requires_explicit_operator_approval=true`, `would_insert=false`, and `write_allowed=false`; this parser exists to freeze the expected-hash input surface before any approval success path or write is considered, not to enable audit rows. Live restore and audit row writes remain unavailable until a separate explicit policy slice, and broad mutation remains blocked — DO NOT enable broad G4 apply mode.
