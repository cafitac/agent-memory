...
## Current G4a safety hardening: restore audit write expected approval hash missing contract

`dogfood query-preview-cleanup-restore --apply` remains read-only and blocked. After v0.1.117 locked the invalid/unverified token boundary, the current slice adds only an expected-approval-hash-missing contract for the future audit write path: a supplied token is accepted as secret input, never printed raw, represented only by `approval_token_sha256`, marked `approval_token_present=true`, but cannot validate unless an expected approval hash exists. With no expected hash configured, the contract reports `approval_token_expected_sha256_required=true`, `approval_token_expected_sha256_present=false`, `approval_token_validation_status=expected_hash_missing`, `approval_token_validated=false`, `write_blocked_by_missing_expected_approval_hash=true`, and `restore_audit_write_approval_token_expected_hash_missing`. Without the token flag, the missing-token fail-closed contract remains in force. Both paths must still report `requires_explicit_operator_approval=true`, `would_insert=false`, and `write_allowed=false`; this negative contract exists to freeze the "token alone is never enough" boundary before any approval success path or write is considered, not to enable audit rows. Live restore and audit row writes remain unavailable until a separate explicit policy slice, and broad mutation remains blocked — DO NOT enable broad G4 apply mode.
