...
## Current G4a safety hardening: restore audit write approval-token dry-run parser

`dogfood query-preview-cleanup-restore --apply` remains read-only and blocked. After v0.1.115 locked the approval-token-missing negative gate, the current slice adds only an approval-token flag dry-run parser for the future audit write path: a supplied token is accepted as secret input, never printed raw, represented only by `approval_token_sha256`, marked `approval_token_present=true`, and still blocked with `approval_token_validated=false`, `write_blocked_by_unvalidated_approval=true`, and `restore_audit_write_approval_token_unvalidated`. Without the flag, the missing-token fail-closed contract remains in force. Both paths must still report `requires_explicit_operator_approval=true`, `would_insert=false`, and `write_allowed=false`; this parser exists to freeze the token-present contract before any approval validation or write is considered, not to enable audit rows. Live restore and audit row writes remain unavailable until a separate explicit policy slice, and broad mutation remains blocked — DO NOT enable broad G4 apply mode.
