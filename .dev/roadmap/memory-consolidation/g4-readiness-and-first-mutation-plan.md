...
## Current G4a safety hardening: restore audit write row materialization

`dogfood query-preview-cleanup-restore --apply` remains read-only and blocked. After v0.1.112 locked duplicate/conflict fail-closed preflight behavior, the current slice adds only a dry-run `experience_traces` row materialization contract for that future audit write path: schema version, target table, ordered columns, canonical stringified DB values, duplicate key, content/metadata hashes, and privacy flags. The materialization must still report `would_insert=false` and `write_allowed=false`; it exists to freeze the final row shape before any write is considered, not to enable audit rows. Live restore and audit row writes remain unavailable until a separate explicit policy slice, and broad mutation remains blocked — DO NOT enable broad G4 apply mode.
