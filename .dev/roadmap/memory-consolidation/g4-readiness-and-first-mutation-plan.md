...
## Current G4a safety hardening: restore audit write dry-run contract

`dogfood query-preview-cleanup-restore --apply` remains read-only and blocked. The current slice adds only the future audit write dry-run shape for a restore operation: `experience_traces` target table/event kind/retention policy, deterministic content and metadata hashes, and an aggregate metadata preview containing policy, actor, `reason_sha256`, artifact hash, source DB fingerprint, source/integrity booleans, disposable rehearsal status, restored ids hash, and restored count. It must report `would_insert=false` and must not include raw `query_preview`, raw reason, samples, tokens, or API-key-like strings. Live restore and audit row writes remain unavailable until a separate explicit policy slice.
