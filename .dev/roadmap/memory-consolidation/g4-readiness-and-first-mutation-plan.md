...
## Current G4a safety hardening: restore audit write policy packet

`dogfood query-preview-cleanup-restore --apply` remains read-only and blocked. After v0.1.113 locked the dry-run `experience_traces` row materialization shape, the current slice adds only a single-row apply policy packet for that future audit write path: exact required policy, actor, reason hash, source DB fingerprint, artifact hash, rehearsal status, preflight pass state, duplicate count, expected insert count, row materialization hash/schema, rollback/manual-review note, and privacy flags. The packet must still report `requires_explicit_operator_approval=true`, `would_insert=false`, and `write_allowed=false`; it exists to freeze the operator approval payload before any write is considered, not to enable audit rows. Live restore and audit row writes remain unavailable until a separate explicit policy slice, and broad mutation remains blocked — DO NOT enable broad G4 apply mode.
