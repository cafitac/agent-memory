...
## Current G4a safety hardening: restore audit write approval-token hash mismatch contract

`dogfood query-preview-cleanup-restore --apply` remains read-only and blocked. After v0.1.119 locked the expected-hash parser while keeping validation not implemented, the current slice adds only a token/expected-hash mismatch negative contract for the future audit write path: when `--approval-token` and `--approval-token-expected-sha256` are both supplied but the token SHA-256 does not equal the normalized expected hash, the contract reports `approval_token_hash_matches_expected=false`, `approval_token_validation_status=hash_mismatch`, `approval_token_validated=false`, `write_blocked_by_approval_hash_mismatch=true`, and `restore_audit_write_approval_token_hash_mismatch`. A supplied token is still never printed raw and remains represented only by `approval_token_sha256`; the expected hash remains normalized plus fingerprinted. Matching token/expected-hash input remains validation-not-implemented until a separate explicit validator-success slice. All paths must still report `requires_explicit_operator_approval=true`, `would_insert=false`, and `write_allowed=false`; this mismatch contract exists to freeze the wrong-token boundary before any approval success path or write is considered, not to enable audit rows. Live restore and audit row writes remain unavailable until a separate explicit policy slice, and broad mutation remains blocked — DO NOT enable broad G4 apply mode.
