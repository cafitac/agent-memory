...
## Current G4a safety hardening: restore audit write invalid-token validator negative contract

`dogfood query-preview-cleanup-restore --apply` remains read-only and blocked. After v0.1.116 locked the token-present parser, the current slice adds only an invalid-token validator negative contract for the future audit write path: a supplied token is accepted as secret input, never printed raw, represented only by `approval_token_sha256`, marked `approval_token_present=true`, explicitly classified as `approval_token_validation_status=invalid_or_unverified`, and still blocked with `approval_token_validated=false`, `write_blocked_by_unvalidated_approval=true`, `write_blocked_by_invalid_approval=true`, and `restore_audit_write_approval_token_invalid`. Without the flag, the missing-token fail-closed contract remains in force. Both paths must still report `requires_explicit_operator_approval=true`, `would_insert=false`, and `write_allowed=false`; this negative validator exists to freeze the invalid/unverified token fail-closed boundary before any approval success path or write is considered, not to enable audit rows. Live restore and audit row writes remain unavailable until a separate explicit policy slice, and broad mutation remains blocked — DO NOT enable broad G4 apply mode.
