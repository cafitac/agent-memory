...
- Restore apply intent is contract-only: it requires `legacy-query-preview-cleanup-restore-v1`, actor, reason hash, source/integrity gates, and a private disposable-restore rehearsal while returning `mutated=false` and `restore_apply_available=false`.
